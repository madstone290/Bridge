    ...
    public class ExceptionFilter : IExceptionFilter
    {
        ...
        public void OnException(ExceptionContext context)
        {
            if (context.Exception is DomainException domainException)
            {
                ...
                context.Result = new BadRequestObjectResult(new ApiError(domainException.Message));
                return;
            }
            ...
            if (context.Exception is AppException appException)
            {
                ...
                context.Result = new BadRequestObjectResult(new ApiError(appException.Message, appException.Code));
                return;
            }
            ...
            if (context.Exception is DbUpdateException updateException)
            {
                if (updateException.InnerException is PostgresException postgresException)
                {
                    ...
                    {
                        context.Result = new BadRequestObjectResult(new ApiError("외래키 제약 조건을 위반하였습니다", ErrorCodes.FOREIGN_KEY_VIOLATION));
