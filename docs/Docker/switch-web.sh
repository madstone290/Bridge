...
# CONST NO OVERRIDE
DOCKER_IMAGE=madstone290/bridge-web:latest
DOCKER_NETWORK=nginx_group
COLOR_ENDPOINT=devops/color
BLUE_IP=172.20.0.21
GREEN_IP=172.20.0.22
NGINX_CONTAINER=nginx
BLUE_CONTAINER=bridge-web-blue
GREEN_CONTAINER=bridge-web-green
BLUE_URL=http://$BLUE_IP/$COLOR_ENDPOINT
GREEN_URL=http://$GREEN_IP/$COLOR_ENDPOINT
WORK_DIR=/home/ubuntu/docker/bridge/
BLUE_CONFIG=${WORK_DIR}bridge-web-blue.conf
GREEN_CONFIG=${WORK_DIR}bridge-web-green.conf
...
deployBlue(){
	...
	docker run -d --name $BLUE_CONTAINER --network $DOCKER_NETWORK --ip $BLUE_IP $DOCKER_IMAGE COLOR=$BLUE API_URL=http://api.placechart.com/
}
...
deployGreen(){
	...
	docker run -d --name $GREEN_CONTAINER --network $DOCKER_NETWORK --ip $GREEN_IP $DOCKER_IMAGE COLOR=$GREEN API_URL=http://api.placechart.com/
}
...
switch(){
	...
	docker cp $config_path $NGINX_CONTAINER:/etc/nginx/conf.d/bridge-web.conf
